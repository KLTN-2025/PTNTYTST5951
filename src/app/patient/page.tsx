...
export default function Home() {
  const { data: patientInfo, isFetching, isError, error } = usePatient();
  useEffect(() => {
    if (isError) {
  ...
  if (patientInfo) {
      ...
      <div className="w-full flex flex-col gap-4">
        <div className="w-full bg-[#e8f6e9] p-4 rounded-lg flex flex-row justify-between">
          ...
          <div className="bg-white rounded-tl-[55px] rounded-br-[55px] px-8 flex items-center">
            <h1 className="text-primary font-bold">{patientInfo.name}</h1>
          </div>
        </div>
          ...
          <div className="bg-white p-4 rounded-lg shadow-md flex flex-col gap-2 justify-between">
            <div className="flex flex-col gap-2">
              <div className="flex flex-row items-center">
                <CalendarHeart color="#ff914d" className="w-12" />
    ...
    return (
      <div>
        {isFetching && !isError && <span>Đang tải thông tin bệnh nhân...</span>}
        {isError && (
          <span>
            Đã có lỗi xảy ra khi tải thông tin bệnh nhân. Vui lòng đăng xuất và
            đăng nhập lại.
          </span>
        )}
        <Button onClick={() => signOutAction()}>Đăng xuất</Button>
